...
class OrderAcceptanceTest extends BaseAcceptanceTest {
    ...
    private ResultActions 주문_상태를_변경(OrderStatusRequest orderStatusRequest) throws Exception {
        return mvc.perform(MockMvcRequestBuilders.put("/api/orders/{orderId}/order-status", 1L)
    ...
    private void 주문_목록_조회_성공(ResultActions resultActions, OrderRequest order) throws Exception {
        ...
        resultActions.andExpect(MockMvcResultMatchers.status().isOk())
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].orderTable.id").value(order.getOrderTableId()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].orderStatus").value(OrderStatus.COOKING.name()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].orderLineItems[0].seq").value(1L))
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].orderLineItems[0].menuId")
                        .value(orderLineItems.getMenuId()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].orderLineItems[0].quantity")
                        .value(orderLineItems.getQuantity()))
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$.[0].orderLineItems[0].menuName").value("후라이드치킨"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.[0].orderLineItems[0].menuPrice")
                        .value(new BigDecimal(16000.00).floatValue()));
    }

    private ResultActions 주문_목록_조회() throws Exception {
        return mvc.perform(MockMvcRequestBuilders.get("/api/orders")
                .accept(MediaType.APPLICATION_JSON))
                .andDo(MockMvcResultHandlers.print());
    }
    ...
    private void 주문_등록_성공(ResultActions resultActions, OrderRequest 주문) throws Exception {
        ...
        resultActions.andExpect(MockMvcResultMatchers.status().isCreated())
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("orderTable.id").value(주문.getOrderTableId()))
                .andExpect(MockMvcResultMatchers.jsonPath("orderStatus").value(OrderStatus.COOKING.name()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.orderLineItems[0].seq").value(1L))
                .andExpect(
                        MockMvcResultMatchers.jsonPath("$.orderLineItems[0].menuId").value(orderLineItems.getMenuId()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.orderLineItems[0].quantity")
                        .value(orderLineItems.getQuantity()));
    }
