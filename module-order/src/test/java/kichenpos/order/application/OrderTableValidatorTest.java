...
import static kichenpos.order.domain.OrderLineItemsTest.createDuplicateOrderLineItems;
...
class OrderTableValidatorTest {
    ...
    void 조리_또는_식사_중인_주문_테이블이_있을_경우_단체_지정을_해제할_수_없다() {
        // given
        given(orderRepository.existsByOrderTableIdInAndOrderStatusIn(TableGroupServiceTest.createOrderTableIds(), OrderStatus.findNotCompletionStatus()))
                .willReturn(true);

        // when & then
        assertThatThrownBy(() ->
                orderTableValidator.validateUngroup(TableGroupServiceTest.createOrderTableIds())
        ).isInstanceOf(IllegalArgumentException.class)
