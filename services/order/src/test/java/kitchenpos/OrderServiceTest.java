...
class OrderServiceTest {
	...
	void create_happyPath() {
		...
		OrderRequest 주문_요청 = new OrderRequest(-1L, Arrays.asList(주문_항목1_요청, 주문_항목2_요청));
	...
	void create_exceptionCase2() {
		// given
		OrderRequest 주문_요청 = new OrderRequest(-1L, null);
	...
	void list() {
		...
		assertThat(listResponse.get(0).getOrderTableId()).isEqualTo(-1L);
	...
	void changeOrderStatus_happyPath() {
		...
		OrderLineItem 주문_항목1 = new OrderLineItem.Builder().menu(메뉴1).quantity(1L).build();
		OrderLineItem 주문_항목2 = new OrderLineItem.Builder().menu(메뉴2).quantity(2L).build();
		Order 주문 = new Order.Builder().orderTableId(-1L)
