...
public class MenuTest {
    ...
    @BeforeEach
    void setUp() {
        menuProduct_후라이드 = new MenuProduct(TestFixture.상품_후라이드, 1);
        menuProduct_양념 = new MenuProduct(TestFixture.상품_양념, 1);
        menuProducts = Arrays.asList(menuProduct_후라이드, menuProduct_양념);
        totalProductPrice = TestFixture.메뉴상품_후라이드.getProductPrice().multiply(BigDecimal.valueOf(TestFixture.메뉴상품_후라이드.getQuantity()))
