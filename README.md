...
![img.png](img.png)
- 크게 3가지 주요쟁점으로 가져갔습니다.
- 첫째로, OrderLineItem에서 Menu와 양방향 연관관계가 생기는데 사이에 OrderMenu를 추가함으로써 이를 해소했습니다.
- 둘째로, 마찬가지로 OrderTable과 TableGroup사이에서도 발생하는데 이를 단방향 연간관계만 가지도록 수정하였습니다.
- 셋째로, Menu와 MenuProduct사이에서 발생하는 연관관계 역시 단방향 연관관계만 가질수 있도록 객체를 가지는 것이 아닌 Id를 가지도록 수정하였습니다.
