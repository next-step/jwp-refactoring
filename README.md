...
- `주문 테이블`들로 새로운 `단체 지정`을 할 수 있다.
    ...
    - 2개 미만의 `주문 테이블`로 `단체 지정` 할 수 없다.
    - 제시된 `주문 테이블`들 중 하나라도 없으면 `단체 지정`할 수 없다.
    - 비어있지 않은(이미 찬) `주문 테이블`들은 `단체 지정` 할 수 없다.
    - 이미 `단체 지정` 된 `주문 테이블`들은 또 `단체 지정` 할 수 없다.
    - 단체 지정되면 `자리 비움` 상태가 꽉 찬 상태로 바뀐다.
- `단체 지정`을 해제할 수 있다.
...
- [X] OrderTable
  - [X] JPA 적용
  - [X] 비움 상태를 바꿀 수 있는 기능을 OrderTable 도메인 오브젝트로 밀어 넣기
      - [X] OrderAdapter 구현
      - [X] OrderAdapter가 리포지토리가 아닌 서비스 레이어에 의존하도록 변경
        - 리포지토리에 의존할 경우 다른 도메인의 세부사항을 지나치게 많이 알아야 한다.
  - [X] 방문한 손님 수를 바꿀 수 있는 기능을 OrderTable 도메인 오브젝트로 밀어 넣기
    - [X] 손님 수를 0명 이하로 바꾸지 못하도록 하는 기능 도메인으로 이관
    - [X] 단체 지정된 주문 테이블의 손님 수를 바꾸지 못하는 기능 도메인으로 이관
~~- [ ] 모든 어댑터들이 리포지토리가 아닌 서비스 레이어에 의존하도록 변경~~
  - 위의 생각은 반대로 생각함
...
## Step3 피드백 반영
- [X] 테스트 시 fixture는 필요한 부분에서 만들어서 사용하도록 개선
- [X] 테스트 공용 메서드를 afterEach에서 처리하도록 개선
- [X] 테스트 공용 메서드에서 삭제 방식 변경
