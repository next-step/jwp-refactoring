...
* 사용자는 단체를 취소 할 수 있다.
    1. 단체를 조회한다.
    2. 취소 시 주문 상태가 요리중, 식사중 상태가아닌지 체크한다.
    3. 주문테이블의 취소를 한다.
...
* 각 요청과 응답에 대해서 dto를 작성한다.  
  ...
  * 사용자는 게스트의 숫자를 변경 할 수 있다.
  ...
  <테이블 그룹>
  * 사용자는 테이블 그룹을 생성 할 수 있다.
  * 사용자는 테이블 그룹을 취소 할 수 있다.
  
* 서비스 로직의 역할을 다시 한번 체크 한다.
...
* 도메인에서 의존관계를 고려해본다.  (탐색이 반드시 필요한 것인가) -> LazyLoading 이슈를 고려하자
  * 같이 변경 되는것과 그렇지 않은것을 파악한다.
  * 단체 지정을 하면 -> 주문 테이블에 이벤트 핸들러
  * 단체 취소 시 -> 주문 테이블에 이벤트 핸들러
* 마지막으로 패키지 사이클링을 확인해보자. 패키지가 서로 양방향으로 의존하고 있는지를 체크한다.
