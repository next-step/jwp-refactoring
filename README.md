...
* Flyway로 데이터베이스 형상 관리를 수행한다
    * `order_table` 테이블이 갖고 있는 `empty` 컬럼은 영속적으로 관리되어야 하는 데이터(값)이 아닌, 비즈니스 처리 흐름 속에 존재하는 하나의 개념으로 보인다. 따라서 해당 컬럼을 제거하는
      것으로 1차적인 데이터 베이스 형상 관리를 수행한다
