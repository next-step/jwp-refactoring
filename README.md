...
1. 따라서, 처음엔 `MonolithFirst` 로 개발을 시작해야 하고,  
충분한 시간이 흐르고 해당 시스템에 대한 이해도가 어느정도 수준을 넘었을 경우  
`멀티 모듈` 을 고려 해야 하는 지 **의심**해야 한다.

2. 만약 충분한 **의심** 후에 멀티 모듈을 도입하기로 결정했다면,  
의존성에 따라 바운디드 컨텍스트를 명확하게 나눠야 한다.

3. 나누었던 `멀티 모듈`의 역할과 책임이 늘어나게 되면,  
...
- 데이터 주고 받는 관점에서 분리
  ...
  - HTTP 요청 / 응답
  - 이벤트 발행 / 구독 
- [ ] 어딜 어떻게 나눌지에 대해서는 고민을 해봐야 할 것 같습니다..