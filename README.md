...
### Step2 피드백 사항 정리
- 주문 생성 시, 주문 요청 항목의 각 메뉴 조회를 위해 여러번 발생하는 쿼리 개선 
- `주문` <-> `주문 테이블`간의 상호 의존 관계 개선
  - 상호 참조 의존 관계를 단방향 의존관계로 변경
- 주문 시간 항목을 주문 엔티티 생성 시점이 아닌 외부에서 주입
- 상품 엔티티와 메뉴 엔티티가 공용으로 사용하고 있는 가격 객체를 각 도메인의 요구사항만을 표현할 수 있도록 분리
