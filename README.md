...
- 새로운 `메뉴`의 이름, 가격, `메뉴 그룹`, `메뉴 상품들`로 메뉴를 등록할 수 있다.
    - `메뉴`의 가격은 반드시 필요하며, 0원 이상이어야 한다.
    - 존재하는 `메뉴 그룹`만 등록할 수 있다.
    - `메뉴`의 가격은 `메뉴 상품들` 가격의 총합보다 싸거나 같아야 한다.
    - 존재하는 `메뉴 상품들`로만 등록할 수 있다.
    - 존재하는 `상품들`로 구성된 `메뉴 상품들`로만 등록할 수 있다.
...
- `주문 테이블`들로 새로운 `단체 지정`을 할 수 있다.
    ...
    - 제시된 `주문 테이블`이 비었거나 2개 이하면 `단체 지정` 할 수 없다.
