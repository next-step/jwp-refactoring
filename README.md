...
* 주문 테이블이 존재해야 하k고, 비어있지 않은 테이블이어야 한다..
* 주문 생성시 `조리` 상태이다.
#### 주문 상태 변경
* 존재하는 주문만 상태를 변경 가능하다.
* 계산 완료 상태의 주문은 상태 변경이 불가하다.
* 주문 상태 변경시 주문 항목은 바뀌지 않는다.
#### 목록 조회
* 주문 목록을 조회할 수 있다.
...
#### 주문테이블 그룹 해제
* 주문 테이블 그룹 다시 쪼갤 수 있다.
* 주문 테이블 그룹의 주문 테이블이 `조리` 상태이거나, `식사` 상태이면 쪼갤 수 없다.
...
* 메뉴의 가격은 메뉴의 수량 * 메뉴와 연결된 상품의 가격 보다 비쌀 수 없다.
#### 목록 조회
* 메뉴 목록을 조회할 수 있다.
