...
### 2단계 피드백
- [ ] API 스펙은 유지하면서 리펙터링 진행할 것 (API 스펙이 바뀐다는 것은 클라이언트 측에서 끔찍한 일)
- [ ] 미사용 의존성 제거
- [ ] `MenuRequest` 에서 validation 적용 필요(request 내에서 List 타입이 있는 경우 전수 조사 필요)
- [ ] `Objects.isNull` 메서드 사용은 `Predicate` 이외에는 지양할 것 (가독성이 떨어짐, 차라리 `null == price`로 사용)
