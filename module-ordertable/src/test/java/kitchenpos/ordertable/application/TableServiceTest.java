...
import kitchenpos.ordertable.validator.AlreadyGroupedOrderTableValidator;
import kitchenpos.ordertable.validator.OrderStatusOrderTableValidator;
import kitchenpos.ordertable.validator.OrderTableValidator;
import kitchenpos.ordertable.validator.OrderTableValidatorsImpl;
...
class TableServiceTest {
    ...
    void 주문_테이블을_등록할_수_있다() {
        BDDMockito.given(orderTableRepository.save(ArgumentMatchers.any())).willReturn(orderTable);
    ...
    void 주문_테이블_목록을_조회할_수_있다() {
        BDDMockito.given(orderTableRepository.findAll()).willReturn(Collections.singletonList(orderTable));

        List<OrderTable> orderTables = tableService.list();

        assertAll(
                () -> Assertions.assertThat(orderTables).hasSize(1),
                () -> Assertions.assertThat(orderTables).containsExactly(orderTable)
        );
    }
    ...
    void 등록_된_주문_테이블에_대해서만_비어있음_여부를_수정할_수_있다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(등록되지_않은_주문테이블_수정);
    }
    ...
    void 이미_단체_지정이_된_주문테이블은_수정할_수_없다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(이미_단체_지정이_된_주문테이블_수정);
    }
    ...
    void 조리_식사_상태의_주문이_포함되어_있으면_수정할_수_없다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(조리_식사_상태의_주문이_포함_된_주문테이블_수정);
    }
    ...
    void 주문_테이블의_방문한_손님수를_0명_이하로_수정할_수_없다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(손님수_0명_이하로_수정);
    }
    ...
    void 등록_된_주문_테이블에_대해서만_방문한_손님수를_수정할_수_있다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(등록되지_않은_주문_테이블_수정);
    }

    @Test
    void 빈_테이블은_수정할_수_없다() {
        OrderTable 빈_주문_테이블 = new OrderTable(1, true);
        BDDMockito.given(orderTableRepository.findById(ArgumentMatchers.any())).willReturn(Optional.of(빈_주문_테이블));

        ThrowingCallable 빈_테이블_수정 = () -> tableService.changeNumberOfGuests(1L, numberOfGuestsRequest);

        Assertions.assertThatIllegalArgumentException().isThrownBy(빈_테이블_수정);
    }
}
