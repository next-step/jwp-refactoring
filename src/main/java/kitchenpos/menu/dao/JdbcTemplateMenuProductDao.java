...
public class JdbcTemplateMenuProductDao implements MenuProductRepository {
    ...
    public List<MenuProduct> findAll() {
        final String sql = "SELECT seq, product_id, quantity FROM menu_product";
        return jdbcTemplate.query(sql, (resultSet, rowNumber) -> toEntity(resultSet));
    }

    @Override
    public List<MenuProduct> findAllByMenuId(final Long menuId) {
        final String sql = "SELECT seq, product_id, quantity FROM menu_product WHERE menu_id = (:menuId)";
    ...
    private MenuProduct select(final Long id) {
        final String sql = "SELECT seq, product_id, quantity FROM menu_product WHERE seq = (:seq)";
        final SqlParameterSource parameters = new MapSqlParameterSource()
                .addValue("seq", id);
