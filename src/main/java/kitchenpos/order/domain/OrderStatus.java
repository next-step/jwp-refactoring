...
public enum OrderStatus {
    ...
    public static void validStatusIsCookingOrMealThrow(OrderStatus orderStatus) {
        if (asList(COOKING, MEAL).contains(orderStatus)) {
            throw new IllegalArgumentException("주문 상태가 조리 또는 식사 상태인 경우에는 단체지정할 수 없습니다.");
        }
    }

    public static void validStatusIsCookingOrMealThrow(List<OrderStatus> orderStatus) {
        orderStatus.stream()
