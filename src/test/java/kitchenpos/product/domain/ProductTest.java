...
class ProductTest {
	...
	public void createProductTest() {
		//when
		Product product = new Product("로제치킨", new BigDecimal(15000));
		//then
		assertThat(product).isNotNull();
		assertThat(product).isEqualTo(new Product("로제치킨", new BigDecimal(15000)));
	}
	...
	public void createProductFailPriceLessThanZeroTest() {
		//when
		assertThatThrownBy(() -> new Product("로제치킨", new BigDecimal(-1)))
			.isInstanceOf(IllegalArgumentException.class)
			.hasMessage("상품 가격은 0이상의 값을 가져야합니다.");
	}
