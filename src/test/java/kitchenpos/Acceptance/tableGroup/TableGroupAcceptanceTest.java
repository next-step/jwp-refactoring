package kitchenpos.Acceptance.tableGroup;

import io.restassured.response.ExtractableResponse;
import io.restassured.response.Response;
import kitchenpos.Acceptance.AcceptanceTest;
import kitchenpos.Acceptance.utils.RestAssuredRequest;
import kitchenpos.domain.Order;
import kitchenpos.domain.OrderStatus;
import kitchenpos.domain.OrderTable;
import kitchenpos.domain.TableGroup;
import org.apache.http.HttpStatus;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;
import java.util.List;
import java.util.stream.Collectors;

import static kitchenpos.Acceptance.order.OrderAcceptanceTest.주문_상태_변경_요청;
import static kitchenpos.Acceptance.order.OrderAcceptanceTest.주문_테이블에_새로운_주문_생성;
import static kitchenpos.Acceptance.table.TableAcceptanceTest.빈_테이블_변경_요청;
import static kitchenpos.Acceptance.table.TableAcceptanceTest.테이블_생성_요청;
import static kitchenpos.domain.OrderTableTest.주문_테이블_생성;
import static kitchenpos.domain.OrderTest.주문_생성;
import static kitchenpos.domain.TableGroupTest.테이블_그룹_생성;
import static org.assertj.core.api.Assertions.assertThat;

public class TableGroupAcceptanceTest extends AcceptanceTest {
    private static final String PATH = "/api/table-groups";

    private List<OrderTable> orderTables = new ArrayList<>();
    private OrderTable notSavedOrderTable = 주문_테이블_생성(null, 1, true);

    @BeforeEach
    public void setUp() {
        super.setUp();

        orderTables.clear();
        for(int i = 0; i < 10; i++) {
            orderTables.add(테이블_생성_요청(i).as(OrderTable.class));
        }
    }

    @DisplayName("테이블 그룹 생성 시 해당 그룹에 속한 테이블의 수가 2개 미만이면 예외가 발생해야 한다")
    @Test
    void createTableGroupByNotFillMinimumTableCount() {
        // when
        ExtractableResponse<Response> 테이블_그룹에_속한_테이블이_한개_요청_결과 = 테이블_그룹_생성_요청(Collections.singletonList(orderTables.get(0)));
        ExtractableResponse<Response> 테이블_그룹에_속한_테이블이_없는_요청_결과 = 테이블_그룹_생성_요청(Collections.emptyList());

        // then
        테이블_그룹_생성_요청_실패됨(테이블_그룹에_속한_테이블이_한개_요청_결과);
        테이블_그룹_생성_요청_실패됨(테이블_그룹에_속한_테이블이_없는_요청_결과);
    }

    @DisplayName("테이블 그룹의 테이블 수와 실제 저장된 테이블 수가 다를 경우 예외가 발생해야 한다")
    @Test
    void createTableGroupByNotMatchedTableCountTest() {
        // when
        ExtractableResponse<Response> 생성_요청_결과 = 테이블_그룹_생성_요청(Arrays.asList(orderTables.get(0), notSavedOrderTable));

        // then
        테이블_그룹_생성_요청_실패됨(생성_요청_결과);
    }

    @DisplayName("테이블 그룹 생성 시 포함된 테이블 중 빈 테이블 아니거나 이미 다른 테이블 그룹에 속해 있는 테이블이 포함되어 있으면 예외가 발생해야 한다")
    @Test
    void createTableByContainAlreadyBelongOrNotEmptyTableTest() {
        // given
        테이블_그룹_생성_요청(Arrays.asList(orderTables.get(0), orderTables.get(1)));
        빈_테이블_변경_요청(orderTables.get(2).getId(), 주문_테이블_생성(null, 2, false));

        // when
        ExtractableResponse<Response> 이미_테이블_그룹에_속한_테이블이_포함된_테이블_그룹_생성_요청_결과 =
                테이블_그룹_생성_요청(Arrays.asList(orderTables.get(0), orderTables.get(3)));
        ExtractableResponse<Response> 빈_상태가_아닌_테이블이_포함된_테이블_그룹_생성_요청_결과 =
                테이블_그룹_생성_요청(Arrays.asList(orderTables.get(2), orderTables.get(4)));

        // then
        테이블_그룹_생성_요청_실패됨(이미_테이블_그룹에_속한_테이블이_포함된_테이블_그룹_생성_요청_결과);
        테이블_그룹_생성_요청_실패됨(빈_상태가_아닌_테이블이_포함된_테이블_그룹_생성_요청_결과);
    }

    @DisplayName("정상 상태의 테이블 그룹 생성 요청 시 정상 생성 되어야 한다")
    @Test
    void createTableGroupTest() {
        // given
        List<OrderTable> targetTables = Arrays.asList(orderTables.get(0), orderTables.get(1));

        // when
        ExtractableResponse<Response> 테이블_그룹_생성_요청_결과 = 테이블_그룹_생성_요청(targetTables);

        // then
        테이블_그룹_생성_요청_성공됨(테이블_그룹_생성_요청_결과, targetTables);
    }

    @DisplayName("테이블 그룹에 포함되어 있는 테이블 중 요리중 또는 식사중 상태인 테이블이 존재하는 테이블 그룹을 해제하면 예외가 발생해야 한다")
    @Test
    void ungroupByIncludeCookingOrMealStatusTableTest() {
        // given
        Long 테이블_그룹_아이디 = 테이블_그룹_생성_요청(Arrays.asList(orderTables.get(0), orderTables.get(1))).body().jsonPath().getLong("id");
        주문_테이블에_새로운_주문_생성(orderTables.get(0));
        주문_테이블에_새로운_주문_생성(orderTables.get(1));

        // when
        ExtractableResponse<Response> 테이블_그룹_해제_요청_결과 = 테이블_그룹_해제_요청(테이블_그룹_아이디);

        // then
        테이블_그룹_해제_요청_실패됨(테이블_그룹_해제_요청_결과);
    }

    @DisplayName("정상 상태의 테이블 그룹 해제 요청시 정상 해제 되어야 한다")
    @Test
    void ungroupTestGroupTest() {
        // given
        Long 테이블_그룹_아이디 = 테이블_그룹_생성_요청(Arrays.asList(orderTables.get(0), orderTables.get(1))).body().jsonPath().getLong("id");
        Long 주문_아이디1 = 주문_테이블에_새로운_주문_생성(orderTables.get(0));
        Long 주문_아이디2 = 주문_테이블에_새로운_주문_생성(orderTables.get(1));
        Order 변경될_주문 = 주문_생성(null, OrderStatus.COMPLETION, Collections.emptyList());
        주문_상태_변경_요청(주문_아이디1, 변경될_주문);
        주문_상태_변경_요청(주문_아이디2, 변경될_주문);

        // when
        ExtractableResponse<Response> 테이블_그룹_해제_요청_결과 = 테이블_그룹_해제_요청(테이블_그룹_아이디);

        // then
        테이블_그룹_해제됨(테이블_그룹_해제_요청_결과);
    }

    public static ExtractableResponse<Response> 테이블_그룹_생성_요청(List<OrderTable> orderTables) {
        TableGroup tableGroup = 테이블_그룹_생성(orderTables);

        return RestAssuredRequest.postRequest(PATH, Collections.emptyMap(), tableGroup);
    }

    public static ExtractableResponse<Response> 테이블_그룹_해제_요청(Long tableGroupId) {
        return RestAssuredRequest.deleteRequest(PATH + "/{tableGroupId}", Collections.emptyMap(), tableGroupId);
    }

    void 테이블_그룹_생성_요청_실패됨(ExtractableResponse<Response> response) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.SC_INTERNAL_SERVER_ERROR);
    }

    void 테이블_그룹_생성_요청_성공됨(ExtractableResponse<Response> response, List<OrderTable> belongTables) {
        TableGroup tableGroup = response.as(TableGroup.class);
        List<Long> tableGroupBelongOrderTableIds = response.body().jsonPath().getList("orderTables.id", Long.class);
        List<Long> expectedTableIdes = belongTables.stream().map(OrderTable::getId).collect(Collectors.toList());

        assertThat(response.statusCode()).isEqualTo(HttpStatus.SC_CREATED);
        assertThat(tableGroup.getOrderTables().size()).isEqualTo(belongTables.size());
        assertThat(tableGroupBelongOrderTableIds).containsAll(expectedTableIdes);
        tableGroup.getOrderTables().forEach(orderTable -> assertThat(orderTable.getTableGroupId()).isEqualTo(tableGroup.getId()));
    }

    void 테이블_그룹_해제_요청_실패됨(ExtractableResponse<Response> response) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.SC_INTERNAL_SERVER_ERROR);
    }

    void 테이블_그룹_해제됨(ExtractableResponse<Response> response) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.SC_NO_CONTENT);
    }
}
