...
class ProductRestControllerTest {
    ...
    void findAllProduct() throws Exception {
        ...
        mockMvc.perform(get("/api/products").contentType(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$[0].name", is(후라이드치킨.getName())))
                .andExpect(jsonPath("$[0].price", is(치킨가격)))
                .andExpect(jsonPath("$[1].name", is(양념치킨.getName())))
                .andExpect(jsonPath("$[1].price", is(치킨가격)));
