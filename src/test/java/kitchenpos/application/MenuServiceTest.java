...
class MenuServiceTest {
    ...
    void menu4() {
        when(menuGroupRepository.existsById(any())).thenReturn(true);
        when(productRepository.findById(eq(1L))).thenReturn(Optional.of(new Product("1", BigDecimal.valueOf(200))));
        when(productRepository.findById(eq(2L))).thenReturn(Optional.of(new Product("2", BigDecimal.valueOf(100))));
        List<MenuProduct> menuProducts = Arrays.asList(new MenuProduct(1L, 3L), new MenuProduct(2L, 1L));
