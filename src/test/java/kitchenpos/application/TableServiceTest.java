...
public class TableServiceTest {
    ...
    void 테이블_전체조회() {
        ...
        // mocking
        when(orderTableDao.findAll()).thenReturn(Arrays.asList(첫번째_테이블, 두번째_테이블));

        // when
        List<OrderTable> list = tableService.list();

        // then 
        assertThat(list).containsExactly(첫번째_테이블, 두번째_테이블);
    }
    ...
    void 요리중이거나_식사중이_아니면_빈테이블로_변경_가능() {
        ...
        // mocking
        when(orderTableDao.findById(anyLong())).thenReturn(Optional.of(첫번째_테이블));
        ...
        // when
        OrderTable orderTable = new OrderTable(첫번째_테이블.getId(),
            첫번째_테이블.getTableGroupId(),
            첫번째_테이블.getNumberOfGuests(), true);

        OrderTable 저장된_주문_테이블 = tableService.changeEmpty(첫번째_테이블.getId(), orderTable);

        Assertions.assertThat(저장된_주문_테이블).isEqualTo(첫번째_테이블);
    ...
    void 해당_테이블에_방문한_손님수를_등록() {
        ...
        // when
        OrderTable orderTable = tableService.changeNumberOfGuests(첫번째_테이블.getId(),
            방문자수_3명으로변경);

        Assertions.assertThat(orderTable).isEqualTo(첫번째_테이블);
    }
}
