...
class OrderServiceTest {
    ...
    void setUp() {
        ...
        양념_후라이드_각_한마리_요청 = new OrderRequest(테이블9_사용중.getId(), Arrays.asList(후라이드_한마리_요청, 양념치킨_한마리_요청));


        후라이드_한마리 = new OrderLineItem(후라이드_메뉴, 1);
        양념치킨_한마리 = new OrderLineItem(양념치킨_메뉴, 1);
        양념_후라이드_각_한마리
            = new Order(100L, 테이블9_사용중, COOKING, Arrays.asList(후라이드_한마리, 양념치킨_한마리));
    }
    ...
    void create() {
        // given
        when(tableService.findById(테이블9_사용중.getId())).thenReturn(테이블9_사용중);
        ...
        assertThat(savedOrder.getOrderTable().getId()).isEqualTo(테이블9_사용중.getId());
    ...
    void create_failed1() {
        // given
        OrderRequest 주문내역이_없는_주문 = new OrderRequest(테이블9_사용중.getId(), Collections.emptyList());
        when(tableService.findById(테이블9_사용중.getId())).thenReturn(테이블9_사용중);
    ...
    void list() {
        // given
        Order 양념_후라이드_추가 = new Order(1L, 테이블9_사용중, COOKING, Arrays.asList(후라이드_한마리, 양념치킨_한마리));
    ...
    void changeOrderStatus_failed2() {
        // given
        Order 계산완료_주문 = new Order(1L, 테이블9_사용중, COMPLETION, Arrays.asList(후라이드_한마리, 양념치킨_한마리));
