...
class MenuTest {
    ...
    void noMenuGroupTest() {
        assertThatThrownBy(() -> new Menu(1L, "후라이드치킨", Price.from(16_000), null, new MenuProducts())).isInstanceOf(NoMenuGroupException.class);
    }
    ...
    void sumPriceTest() {
        ...
        assertThatThrownBy(() -> new Menu(1L, "후라이드치킨", Price.from(19_000), new MenuGroup(), menuProducts)).isInstanceOf(IllegalMenuPriceException.class);
    }
    ...
    void negativePriceTest() {
        ...
        Product 후라이드치킨 = new Product(1L, "후라이드치킨", Price.from(16_000));
        MenuProducts menuProducts = new MenuProducts(Collections.singletonList(new MenuProduct(1L, null, 후라이드치킨, 1)));
        assertThatThrownBy(() -> new Menu(1L, "후라이드치킨", Price.from(-1_000), new MenuGroup(), menuProducts)).isInstanceOf(NegativePriceException.class);
    }
}