...
public class TableGroupTest {
    ...
    void 생성_예외_주문_테이블_목록이_빈_경우() {
        assertThatIllegalArgumentException().isThrownBy(() -> 단체_지정(LocalDateTime.now(), OrderTableBag.from(Collections.emptyList())));
    }

    @DisplayName("생성 예외 - 비어 있지 않은 주문 테이블이 포함되어 있을 경우")
    @Test
    void 생성_예외_비어_있지_않은_주문_테이블이_포함되어_있을_경우() {
        assertThatIllegalArgumentException().isThrownBy(() -> 단체_지정(LocalDateTime.now(), OrderTableBag.from(Arrays.asList(
