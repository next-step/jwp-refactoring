...
public class MenuAcceptanceTest extends AcceptanceTest {
    ...
    void 메뉴관리() {
        ...
        response = 메뉴_등록_요청("빅맥버거", BigDecimal.valueOf(3000), 빅맥세트.getId(),
                Arrays.asList(
                        createMenuProductRequest(토마토.getId(), 1),
                        createMenuProductRequest(양상추.getId(), 4)
                )
        );
    ...
    public static ExtractableResponse<Response> 메뉴_등록_요청(String name, BigDecimal price, Long menuGroupId,
        ...
        MenuRequest request = createMenuRequest(name, price, menuGroupId, menuProductRequests);
