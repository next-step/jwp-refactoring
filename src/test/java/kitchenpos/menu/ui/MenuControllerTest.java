...
public class MenuControllerTest extends ControllerTest<MenuRequest> {
    ...
    void 메뉴_생성요청() throws Exception {
        //Given
        when(menuService.create(any())).thenReturn(맥모닝콤보_응답);

        //When
        ResultActions 결과 = postRequest(BASE_URI, 맥모닝콤보_요청);
    ...
    void 메뉴_목록_조회요청() throws Exception {
        //Given
        when(menuService.list()).thenReturn(Arrays.asList(맥모닝콤보_응답, 메뉴_두번째_응답));
