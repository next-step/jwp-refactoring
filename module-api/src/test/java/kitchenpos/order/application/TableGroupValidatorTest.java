...
public class TableGroupValidatorTest {
    ...
    void 주문테이블_목록_비어있는_경우_예외발생() {
        ...
        Mockito.when(orderTableRepository.findAllByIdIn(ArgumentMatchers.any())).thenReturn(Collections.EMPTY_LIST);

        //When + Then
        Throwable 주문테이블_비어있음_예외 = Assertions.catchThrowable(() -> tableGroupValidator.validateGrouping(new TableGroupRequest()));
        Assertions.assertThat(주문테이블_비어있음_예외).isInstanceOf(IllegalArgumentException.class)
    ...
    void 주문테이블_목록_2개_미만_예외발생() {
        ...
        Mockito.when(orderTableRepository.findAllByIdIn(ArgumentMatchers.any())).thenReturn(Arrays.asList(단체지정_안된_비어있지_않은_테이블));

        //When + Then
        Throwable 주문테이블_2개미만_예외 = Assertions.catchThrowable(() -> tableGroupValidator.validateGrouping(테이블그룹_요청));
        Assertions.assertThat(주문테이블_2개미만_예외).isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ERROR_ORDERTABLES_SHOULD_HAVE_AT_LEAST_TWO_TABLES.showText());
    }
    ...
    void 주문테이블이_이미_단체지정된_경우_예외발생() {
        ...
        Mockito.when(orderTableRepository.findAllByIdIn(ArgumentMatchers.any())).thenReturn(테이블목록);


        //When + Then
        Throwable 이미_단체지정된_테이블_예외 = Assertions.catchThrowable(() -> tableGroupValidator.validateGrouping(테이블그룹_요청));
        Assertions.assertThat(이미_단체지정된_테이블_예외).isInstanceOf(IllegalArgumentException.class)
    ...
    void 주문이_완료되지_않은_경우_단체지정_해제_예외발생() {
        ...
        Mockito.when(orderTableRepository.findAllByTableGroupId(1L)).thenReturn(테이블목록);
        Mockito.when(orderRepository.findAllByOrderTableIdIn(ArgumentMatchers.any())).thenReturn(주문목록);

        //When + Then
        Throwable 이미_단체지정된_테이블_예외 = Assertions.catchThrowable(() -> tableGroupValidator.validateUngrouping(1L));
        Assertions.assertThat(이미_단체지정된_테이블_예외).isInstanceOf(CannotUpdateException.class)
