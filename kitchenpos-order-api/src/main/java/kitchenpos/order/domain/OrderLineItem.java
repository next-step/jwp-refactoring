...
public class OrderLineItem {
    ...
    private void validate(Order order, OrderMenu orderMenu, long quantity) {
        if (Objects.isNull(order)) {
            throw new IllegalArgumentException(MenuExceptionConstants.INVALID_FORMAT_ORDER.getErrorMessage());
        }
        ...
        if (isNotPositiveNumber(quantity)) {
            throw new IllegalArgumentException(MenuExceptionConstants.INVALID_QUANTITY.getErrorMessage());
        }
    }
