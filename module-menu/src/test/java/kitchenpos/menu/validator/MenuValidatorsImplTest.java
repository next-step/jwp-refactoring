...
class MenuValidatorsImplTest {
    ...
    void 메뉴_등록시_등록되어_있는_메뉴_그룹만_지정할_수_있다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(등록되지_않은_메뉴_그룹_지정_할_경우);
    }

    @Test
    void 등록되지_않은_상품은_메뉴로_지정할_수_없다() {
        BDDMockito.given(productRepository.findById(ArgumentMatchers.any())).willReturn(Optional.empty());
        BDDMockito.given(menu.getMenuProducts()).willReturn(Collections.singletonList(new MenuProduct(null, 1L, 1)));
    ...
    void 메뉴의_가격은_메뉴_상품들_가격의_합보다_낮아야_한다() {
        ...
        Assertions.assertThatIllegalArgumentException().isThrownBy(메뉴_가격이_상품들_가격의_합보다_높은_경우);
    }
}
