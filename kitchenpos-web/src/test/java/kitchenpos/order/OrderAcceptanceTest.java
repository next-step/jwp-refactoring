...
import static api.menugroup.MenuGroupFixtures.한마리메뉴그룹요청;
...
import static api.table.OrderTableFixtures.주문가능_다섯명테이블요청;
import static api.table.OrderTableFixtures.주문불가_다섯명테이블요청;
...
public class OrderAcceptanceTest extends AcceptanceTest {
    ...
    public void changeOrderStatus() {
        ...
        // when
        ExtractableResponse<Response> response = 주문_상태_변경요청함(savedOrder.getId(), 식사완료로_변경_요청());

        // then
        상태변경됨(response, 식사완료로_변경_요청().getOrderStatus());
    }
    ...
    public void changeOrderStatusFail() {
        // given
        OrderResponse savedOrder = 주문등록되어있음(주문등록요청(주문가능_5인테이블_ID, Lists.newArrayList(주문정보_등록요청(메뉴_ID, 1L))));
        주문_상태_변경요청함(savedOrder.getId(), 식사완료로_변경_요청());

        // when
        ExtractableResponse<Response> response = 주문_상태_변경요청함(savedOrder.getId(), 식사중으로_변경_요청());
    ...
    public static OrderStatusUpdateRequest 식사완료로_변경_요청() {
        return OrderStatusUpdateRequest.of(식사완료());
    }

    public static OrderStatusUpdateRequest 식사중으로_변경_요청() {
        return OrderStatusUpdateRequest.of(식사중());
    }
}
