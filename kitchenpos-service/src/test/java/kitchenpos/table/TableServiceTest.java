...
public class TableServiceTest {
	...
	@Test
	void changeEmpty() {
		when(orderTableRepository.findById(anyLong())).thenReturn(Optional.of(
			테이블1));

		OrderTableResponse orderTableResponse = tableService.changeEmpty(테이블1.getId(),
			new OrderTableRequest(false));
	...
	void changeGuestsNumber() {
		...
		OrderTableResponse orderTableResponse = tableService.changeGuestsNumber(테이블1.getId(),
			new OrderTableRequest(4));
