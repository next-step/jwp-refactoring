...
class TableGroupTest {
	...
	void createTableGroupTest() {
		assertThatNoException()
			.isThrownBy(() -> TableGroup.from(
				OrderTables.from(Arrays.asList(OrderTableGenerator.비어있는_다섯명_테이블(), OrderTableGenerator.비어있는_두명_테이블()))));
	}
	...
	void createTableGroupWithLessThanTwoOrderTableTest() {
		assertThatIllegalArgumentException()
			.isThrownBy(() -> TableGroup.from(
				OrderTables.from(Collections.singletonList(OrderTableGenerator.비어있는_다섯명_테이블()))
			))
	...
	void createTableGroupWithNotEmptyOrderTableTest() {
		assertThatIllegalArgumentException()
			.isThrownBy(() -> TableGroup.from(
				OrderTables.from(Arrays.asList(OrderTableGenerator.비어있지_않은_5명_테이블(), OrderTableGenerator.비어있는_두명_테이블()))
			))
